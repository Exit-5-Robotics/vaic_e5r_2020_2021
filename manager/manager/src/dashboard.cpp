...
dashboardJetson( int ox, int oy, int width, int height ) {
  ...
  for(int i=0;i<4;i++ ) {
    if( i < local_map.boxnum ) {
      std::string x = patch::to_string(local_map.boxobj[i].x);
      std::string y = patch::to_string(local_map.boxobj[i].y);
      std::string w = patch::to_string(local_map.boxobj[i].width);
      std::string h = patch::to_string(local_map.boxobj[i].height);
      std::string d = patch::to_string(local_map.boxobj[i].depth);
